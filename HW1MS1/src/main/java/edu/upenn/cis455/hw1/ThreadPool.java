...
/**
 * ThreadPool class that monitors all thread and can query status of each thread
 * @author Kushagra
 *
 * @param <T> any class that is derived from a Socket.
 */
public class ThreadPool<T extends Socket> {
	static final Logger logger = Logger.getLogger(ThreadPool.class);
	private BlockingQueue<T> queue = null;
	private List<WorkerThread<T>> threads = new ArrayList<WorkerThread<T>>();
	private int port;
	 ...
	 */
	public ThreadPool(int maxNumberOfActiveConnections, int maxNumberOfThreads, Path rootDirectory, int p, ServerSocket sc, HttpServer si) {
		port = p;
		serverSocket = sc;
		serverInstance = si;
		queue = new BlockingQueue<T>(maxNumberOfActiveConnections);
		for(int i = 0; i < maxNumberOfThreads; i++) {
			threads.add(new WorkerThread<T>(queue,  rootDirectory, i + 1, this, port));
		}
	 ...
	 */
	public synchronized void enqueueConnection(T s) {
		try {
			queue.enqueue(s);
	 ...
	 * @throws Exception
	 */
	public synchronized void shutdown(Socket s, boolean isHead) throws Exception {
	...
	public synchronized void  sendControlPage(Socket s, boolean isHead) throws Exception {
		BufferedOutputStream out = new BufferedOutputStream(s.getOutputStream());
		PrintWriter p = new PrintWriter(out, true);
		String dateNow = Instant.now().atOffset(ZoneOffset.UTC).format(DateTimeFormatter.RFC_1123_DATE_TIME);
		logger.info("Sending Control Page");
		...
		if(HttpServer.useHTTPs) {
			htmlControlPage.add(String.format("<button onclick=\"window.location.href = 'https://localhost:%d/shutdown';\">Shutdown</button>", port));			
		}
		else {
